...
namespace Chess.Game {
	public class BoardUI : MonoBehaviour {
		...
		public GameObject Chess_Square;
		...
		MeshFilter[, ] squarePieceMeshFilters;
		Move lastMadeMove;
		...
		public void DragPiece (Coord pieceCoord, Vector2 mousePos) {
			...
			squarePieceMeshFilters[pieceCoord.fileIndex, pieceCoord.rankIndex].transform.position = new Vector3(mousePos.x, mousePos.y, pieceDragDepth);
        }
		...
		public void ResetPiecePosition(Coord pieceCoord) {
			...
			squarePieceMeshFilters[pieceCoord.fileIndex, pieceCoord.rankIndex].transform.position = pos;
		...
		public bool TryGetSquareUnderMouse (Vector3 mouseWorld, out Coord selectedCoord) {
			int file = (int) (mouseWorld.x + 4);
			int rank = (int) (mouseWorld.z + 4);
			if (!whiteIsBottom) {
				file = 7 - file;
				rank = 7 - rank;
			}
		...
		public void UpdatePosition (Board board) {
			for (int rank = 0; rank < 8; rank++) {
				for (int file = 0; file < 8; file++) {
					...
					squarePieceMeshFilters[file, rank].mesh = pieceTheme.GetPieceMesh(piece);
					Material material = new Material(Shader.Find("Standard"));
					material.color = Piece.IsColour(piece, Piece.White)? Color.white : Color.black;
                    squarePieceMeshFilters[file, rank].GetComponent<MeshRenderer>().material = material;
                   // squarePieceMeshFilters[file, rank].GetComponent<MeshRenderer>().material = new Material(Shader.Find("Standard"));

                    squarePieceMeshFilters[file, rank].transform.position = PositionFromCoord (file, rank, pieceDepth);
				}
			}
		...
		IEnumerator AnimateMove (Move move, Board board) {
			...
			Transform pieceT2 = squarePieceMeshFilters[startCoord.fileIndex, startCoord.rankIndex].transform;
			...
			while (t <= 1) {
				...
				pieceT2.position = Vector3.Lerp (startPos, targetPos, t);
			}
			UpdatePosition (board);
			ResetSquareColours ();
			pieceT.position = startPos;
			pieceT2.position = startPos;
		}
		...
		void CreateBoardUI () {
			...
			squarePieceMeshFilters = new MeshFilter[8, 8];
			...
			for (int rank = 0; rank < 8; rank++) {
				for (int file = 0; file < 8; file++) {
                    ...
                    square.rotation = Quaternion.Euler(90, 0, 0); // Rotate the square to be flat
					...
					//3D Square
					GameObject Square = Instantiate(Chess_Square, transform);
					Square.transform.position = PositionFromCoord (file, rank, 0);
					Square.name = BoardRepresentation.SquareNameFromCoordinate(file, rank);
					// PASSER EN PREFAB AVEC SCRIPT + ENTRER DANS UN TABLEAU.
                    ...
                    pieceRenderer.transform.rotation = Quaternion.Euler(90, 0, 0); // Rotate the square to be flat
					pieceRenderer.transform.localScale = Vector3.one * 100 / (2000 / 6f);
					squarePieceRenderers[file, rank] = pieceRenderer;

					//Cerare piece mesh renderer for current square
					MeshFilter pieceRenderer2 = new GameObject("Piece").AddComponent<MeshFilter>();
					pieceRenderer2.AddComponent<MeshRenderer>();
                    pieceRenderer2.transform.parent = square;
                    pieceRenderer2.transform.position = PositionFromCoord(file, rank, pieceDepth);
					squarePieceMeshFilters[file, rank] = pieceRenderer2;
                }
			}
		...
		public Vector3 PositionFromCoord (int file, int rank, float depth = 0) {
			if (whiteIsBottom) {
                ...
                return new Vector3(-3.5f + file, depth, -3.5f + rank);
            }
            //return new Vector3 (-3.5f + 7 - file, 7 - rank - 3.5f, depth);
            return new Vector3(-3.5f + 7 - file, depth, 7 - rank - 3.5f);
        }
